...
function annotateFlags(flagStore, branch, results, dateLabel, dryRun = false, cycleId = dateLabel) {
  ...
  for (const r of results) {
    ...
    if (r.match) {
      // Only the same audit cycle may resolve its own discrepancy.
      // A later unrelated shift/day that happens to balance must not clear it.
...
function buildShiftAuditReport({ branchConfig, closingCount, openingCount, results, tickets, expenseEntries = [], dryRun = false }) {
  ...
  for (const r of resolved) {
    lines.push(`✅ ${r.ccy} resolved — corrected closing cash count now reconciles.`);
  }
...
function buildQuestionReport({ branchConfig, title, dateLabel, windowText, openingTeller,
  ...
  for (const r of resolved) {
    lines.push('');
    lines.push(`✅ ${r.ccy} handover now reconciles.`);
  }
